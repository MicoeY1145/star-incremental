...
let offlineProcessedTime = 0;

// 每帧额外处理多少秒离线时间
const OFFLINE_CHUNK_PER_FRAME = 5;
...
let generaterCountEl, generaterCostEl, generaterProductionEl,buyGeneraterButton;
let resetButton;
...
/*离线弹框控制函数*/
function startOfflineProcessing(seconds) {
    const safeSeconds = Math.max(0, Number(seconds) || 0);

    offlineBacklog = safeSeconds;
    offlineTotalTime = safeSeconds;
    offlineProcessedTime = 0;
    isOfflineProcessing = safeSeconds > 0.1;

    updateOfflinePopup();
}


function finishOfflineProcessing() {
    offlineBacklog = 0;
    offlineProcessedTime = offlineTotalTime;
    ...
    showNotification("离线进度计算完成");
}


function skipOfflineProcessing() {
    ...
    showNotification("已跳过剩余离线收益");
}

function updateOfflineInteractionLock() {
    const selectors = [
        "#click-button",
        "#buy-stardustCondenser",
        "#buy-stardustCollector",
        "#buy-stardustGenerater",
        ".buy-upgrade",
        "#reset-button",
        "#import-save-button",
        "#transformer-panel button",
        "#transformer-input"
    ];
    ...
    document
        ...
        .forEach(element => {
            element.disabled = isOfflineProcessing;
        });
}
...
function updateOfflinePopup() {
    ...
    const total = Math.max(offlineTotalTime, 0);
    const remaining = Math.max(offlineBacklog, 0);
    ...
    const processedElement = document.getElementById(
        "offline-processed-seconds"
    );

    const remainingElement = document.getElementById(
        "offline-remaining-seconds"
    );
    ...
    if (processedElement) {
        processedElement.textContent = Math.floor(processed);
    }

    if (remainingElement) {
        remainingElement.textContent = Math.ceil(remaining);
    }
...
function gameLoop() {
    ...
    // 防止浏览器异常卡顿产生巨大实时 delta
    realDelta = Math.min(
        Math.max(realDelta, 0),
        MAX_FRAME_DT
    );

    let deltaTime = realDelta;
     ...
     */
    if (isOfflineProcessing) {
        const offlineChunk = Math.min(
            offlineBacklog,
            OFFLINE_CHUNK_PER_FRAME
        );

        deltaTime = offlineChunk;

        offlineBacklog = Math.max(
            0,
            offlineBacklog - offlineChunk
        );

        offlineProcessedTime += offlineChunk;
    }

    if (transformerActive) {
        transformerTimeLeft = sub(
            transformerTimeLeft,
            deltaTime
        );
    ...
    stardust = add(
        stardust,
        mult(stardustPerSecond, deltaTime)
    );
    ...
    if (saveData) {
        ...
        if (saveData.lastUpdateTime) {
            const elapsedSeconds =
                Math.max(
                    0,
                    Date.now() - Number(saveData.lastUpdateTime)
                ) / 1000;
            ...
            // 少于1秒时不显示离线弹框
            if (cappedOfflineTime >= 1) {
                startOfflineProcessing(cappedOfflineTime);
            } else {
                offlineBacklog = 0;
                isOfflineProcessing = false;
            }
...
function initEventListeners() {
    ...
    // 离线进度跳过按钮
    const skipBtn =
    document.getElementById("skip-offline-button");

    if (skipBtn) {
        skipBtn.addEventListener(
            "click",
            skipOfflineProcessing
        );
    }
}
